    ...
    ITokenService tokenService) : IRequestHandler<LoginUserCommand, Either<BaseException, string>>
{
    ...
    {
        var user = await userQueries.GetByIdWithDetailsAsync(
            (await userQueries.GetByEmailAsync(request.Email, cancellationToken))
            .Match(u => u.Id, () => 0), cancellationToken);

        var foundUser = await userQueries.GetByEmailAsync(request.Email, cancellationToken);

        return foundUser.Match<Either<BaseException, string>>(
